...
func (gc *GameController) GetParentCategoryList(ctx context.Context) (*pb.CategoryListResponse_CategoryList, error) {
	parentCategoryList, err := gc.gr.GetAllCategoryList(ctx)
	if err != nil {
		return nil, err
	}
...
func (gc *GameController) GetGameListByCategory(ctx context.Context, categoryName string, page, size int32, sort string) (*pb.GameSimpleListResponse_GameSimpleList, error) {
	...
	for i, game := range gameSimpleList {
		categoryList, err := gc.gr.GetCategoryListByGameId(ctx, game.Id)
		if err != nil {
			return nil, err
		}
...
func (gc *GameController) GetGameDetail(ctx context.Context, gameId int32) (*pb.GameDetail, error) {
	...
	categoryList, err := gc.gr.GetCategoryListByGameId(ctx, int(gameId))
	if err != nil {
		return nil, err
	}
	categoryTmp := make([]string, len(categoryList))
	...
	gamePublisher, err := gc.gr.GetPublisher(ctx, gameDetail.PublisherId)
	if err != nil {
		return nil, err
	}
